...
from tilt import calculate_tilt, process_site
...
class ResultFilter(QComboBox):
    def __init__(self, parent=None):
        ...
        self.addItem("Tilt Issue Cells")

class TiltThreshold(QSpinBox):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setMinimum(1)
        self.setMaximum(20)  # Maximum tilt of 20 degrees
        self.setValue(4)  # Default threshold of 4 degrees
...
class MetricCard(QFrame):
    ...
    def get_icon_path(self, title):
        ...
        try:
            ...
            icon_mapping = {
                "Cells Tilt Greater Than Threshold": "inter-freq-icon.svg",
...
class TiltTable(QFrame):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setObjectName("tiltTable")
        self.setStyleSheet("""
            QFrame#tiltTable {
                background-color: white;
                border-radius: 10px;
    ...
    def setup_ui(self):
        ...
        self.table.setHorizontalHeaderLabels([
            "eNodeB Name", "Cell ID", "Carrier", "Actual Tilt"
        ])
...
class ActualTiltWindow(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        ...
        self.threshold_value = 4  # Default threshold value
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        self.tilt_table = TiltTable()
        content.addWidget(self.tilt_table, 0, 8, 1, 4)
    ...
    def create_header(self):
        ...
        title = QLabel("Actual Tilt Analysis")
        ...
        threshold_label = QLabel("Tilt Threshold:")
        threshold_label.setStyleSheet("color: #64748b;")
        right_side.addWidget(threshold_label)
        
        self.tilt_threshold = TiltThreshold()
        self.tilt_threshold.valueChanged.connect(self.on_threshold_changed)
        right_side.addWidget(self.tilt_threshold)
    ...
    def analyze_data(self):
        try:
            ...
            progress_dialog.setLabelText("Initializing tilt analysis...")
            ...
            try:
                ...
                # Create base DataFrame with required columns from ep_data
                base_columns = {
                ...
                # Process sites for actual tilt
                grouped = self.mr_data.groupby(self.mappings["MR Site ID"])
                site_results = []
                total_sites = len(grouped)
                
                from tilt import calculate_tilt, calculate_distance
                
                for i, (site_id, group) in enumerate(grouped):
                    for cell_id in group[self.mappings['MR Cell ID']].unique():
                            cell_data = group[group[self.mappings['MR Cell ID']] == cell_id]
                            
                            site_data = self.ep_data[self.ep_data[self.mappings['EP Site ID']] == site_id].iloc[0]
                            site_lat = float(site_data[self.mappings['EP Latitude']])
                            site_lon = float(site_data[self.mappings['EP Longitude']])
                            
                            distances = []
                            rsrp_values = []
                            
                            for _, measurement in cell_data.iterrows():
                                dist = calculate_distance(
                                    site_lat, site_lon,
                                    float(measurement[self.mappings['MR Latitude']]),
                                    float(measurement[self.mappings['MR Longitude']])
                                )
                                if dist > 0:
                                    distances.append(dist)
                                    rsrp_values.append(float(measurement[self.mappings['MR RSRP']]))
                            
                            if distances and rsrp_values:
                                actual_tilt = calculate_tilt(np.array(distances), np.array(rsrp_values))
                                
                                site_results.append({
                                    'Site ID': site_id,
                                    'Cell ID': cell_id,
                                    'Actual Tilt': round(abs(actual_tilt), 2)  # Using absolute value
                                })
                ...
                progress_dialog.setLabelText("Processing tilt data...")
                QApplication.processEvents()
                
                # Convert results to DataFrame and merge
                tilt_df = pd.DataFrame(site_results)
                self.analyzed_df = self.analyzed_df.merge(
                    tilt_df,
                    left_on=['eNodeb Name', 'Cell ID'],
                    right_on=['Site ID', 'Cell ID'],
                    how='left'
                )
                ...
                result_path = os.path.join(results_dir, 'tilt_result.csv')
                ...
                QMessageBox.information(self, "Success", "Tilt analysis completed successfully!")
                
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Failed to analyze tilt data: {str(e)}")
            finally:
                self.is_analyzing = False
                progress_dialog.close()
                    
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Tilt analysis failed: {str(e)}")
            self.is_analyzing = False
    ...
    def calculate_statistics(self):
        ...
        try:
            if hasattr(self, 'analyzed_df') and self.analyzed_df is not None:
                current_threshold = self.tilt_threshold.value()
                stats['total_cells'] = len(self.analyzed_df)
                
                # Use current threshold for tilt issues
                tilt_mask = self.analyzed_df['Actual Tilt'] > current_threshold
                high_tilt_df = self.analyzed_df[tilt_mask]
                stats['high_tilt_count'] = len(high_tilt_df)
                
                if stats['total_cells'] > 0:
                    stats['tilt_percentage'] = (stats['high_tilt_count'] / stats['total_cells']) * 100
                
                stats['affected_sites'] = len(high_tilt_df['eNodeb Name'].unique())
                ...
                for carrier in self.analyzed_df['Carrier'].unique():
                    ...
                    carrier_issues = len(carrier_df[carrier_df['Actual Tilt'] > current_threshold])
    ...
    def create_carrier_chart(self):
        try:
            ...
            for carrier in self.analyzed_df['Carrier'].unique():
                ...
                issues = len(carrier_df[carrier_df['Actual Tilt'] > current_threshold])
            ...
            labels = [stats['label'] for stats in coord_stats.values()]
            ...
            ax.set_title('High Tilt Distribution by Carrier',
                         fontsize=11,
                         pad=10,
                         loc='left',
                         color='#666666')
    ...
    def update_metrics(self):
        try:
            ...
            self.metrics_layout.addWidget(MetricCard(
                "Cells Tilt Greater Than Threshold",
                stats['high_tilt_count'],
                stats['tilt_percentage']
            ))
            self.metrics_layout.addWidget(MetricCard(
    ...
    def update_table(self):
        try:
            ...
            self.tilt_table.set_data(table_data)
    ...
    def apply_result_filter(self, index):
        try:
            ...
            else:  # "Tilt Issue Cells"
                filtered_df = self.analyzed_df[self.analyzed_df['Actual Tilt'] > current_threshold]
    ...
    def on_threshold_changed(self, value):
        try:
            if not self.tilt_threshold.hasFocus():
                self.threshold_value = value
                if self.result_df is not None:
                    QMessageBox.information(
                        self,
                        "Threshold Updated",
                        f"Tilt threshold has been set to {value}°.\nClick 'Analyze' button to update the analysis with new threshold."
                    )
        except Exception as e:
    ...
    def resizeEvent(self, event):
        try:
            ...
            if hasattr(self, 'tilt_table'):
                for col in range(self.tilt_table.table.columnCount()):
                    self.tilt_table.table.resizeColumnToContents(col)
        except Exception as e:
